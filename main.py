...
# 1
# ==========================================
ALGORITHM_VERSION = "frame-diff-single-pipeline-sync-20260619"
ALGORITHM_NOTE = "Use the same static-background intersection and morphology order as single while keeping raw tight-padded ROI input."
N_CAM = 5
INIT_TIME = 5
...
def build_static_bg_model(samples):
    ...
    samples.clear()
    kth = count // 2
    stack.partition(kth, axis=0)
    bg = stack[kth].copy()
    mean = sum_img / float(count)
    variance = np.maximum(sum_sq / float(count) - mean * mean, 0.0)
    std = np.sqrt(variance)
    tol = np.clip(LOW_BG_ABS_DELTA + LOW_BG_STD_MULT * std, LOW_BG_ABS_DELTA, 60.0).astype(np.uint8)
    return bg, tol

def build_initial_video_bg_model(video_path):
    cap = cv2.VideoCapture(video_path)
    if not cap.isOpened():
        raise RuntimeError(f"cannot open video for static background: {video_path}")
    fps = float(cap.get(cv2.CAP_PROP_FPS) or 20.0)
    sample_span = max(1, int(round(STATIC_BG_SECONDS * fps)))
    ...
    bg, tol = build_static_bg_model(samples)
    return bg, tol, sample_count, sample_span
    ...
    def _basic_is_confirmed(self, trk):
        ...
        if trk.get('yolo_hits', 0) >= required_direct_hits:
            return (
                ...
                and (not HIGH_LAYER_MODE or traj_score >= TRACKER_MIN_TRAJ_SCORE)
            )
        return (
...
# 5
# ==========================================
def capture_job(cam_idx, initial_bg_gray=None, initial_bg_tol=None):
    ...
    current_draw_boxes =[]
    bg_samples = []
    bg_gray, bg_tol = initial_bg_gray, initial_bg_tol
    ...
    while not stop_event.is_set():
        ...
        if (f_idx + cam_idx) % PROCESS_EVERY_N_FRAMES == 0:
            ...
            if ENABLE_LOW_LAYER_STATIC_BG_MASK and bg_gray is None:
                ...
                fallback_samples = learning_elapsed >= STATIC_BG_SECONDS + 2.0 and len(bg_samples) >= 20
                if learning_elapsed >= STATIC_BG_SECONDS and (enough_samples or fallback_samples):
            ...
            for roi in rois[:MAX_ROIS_PER_FRAME]:
                ...
                if ENABLE_TIGHT_MOTION_ROI and roi_is_padded_canvas(roi):
...
if __name__ == '__main__':
    initial_bg_gray, initial_bg_tol = None, None
    if SIMULATE_BY_VIDEOS and ENABLE_LOW_LAYER_STATIC_BG_MASK and os.path.isfile(VIDEO_TEST_PATH):
        try:
            initial_bg_gray, initial_bg_tol, bg_sample_count, bg_frame_span = build_initial_video_bg_model(VIDEO_TEST_PATH)
            print(
    ...
    for i in range(N_CAM):
        if use_camera_processes:
            worker = runtime_ctx.Process(
                target=capture_job,
                args=(i, initial_bg_gray, initial_bg_tol),
                daemon=True,
            )
        else:
