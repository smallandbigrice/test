...
# 1
# ==========================================
ALGORITHM_VERSION = "frame-diff-rknn-static-confirm-lowlayer-restore-20260709"
ALGORITHM_NOTE = "Frame-diff RKNN detector with static hover confirmation and restored low-layer ROI scheduling."
...
EDGE_REGION_SUPPRESS_LOW = _env_bool("UAV_EDGE_REGION_SUPPRESS_LOW", False)
EDGE_REGION_FILL_BELOW = _env_bool("UAV_EDGE_REGION_FILL_BELOW", False)
EDGE_REGION_FILL_PAD_PX = max(0, int(os.environ.get("UAV_EDGE_REGION_FILL_PAD_PX", "8")))
EDGE_REGION_DRAW_MASK = _env_bool("UAV_EDGE_REGION_DRAW_MASK", False)
EDGE_REGION_DRAW_ALPHA = min(0.90, max(0.05, float(os.environ.get("UAV_EDGE_REGION_DRAW_ALPHA", "0.28"))))
...
def build_edge_low_region_mask(gray, bg_gray=None):
    ...
    if EDGE_REGION_FILL_BELOW and cv2.countNonZero(low_mask) > 0:
        ys, xs = np.where(low_mask > 0)
        if xs.size > 0:
            skyline = np.full(w, h, dtype=np.int32)
            np.minimum.at(skyline, xs.astype(np.int32), ys.astype(np.int32))
            valid = skyline < h
            if np.any(valid):
                idx = np.arange(w, dtype=np.float32)
                valid_idx = idx[valid]
                valid_y = skyline[valid].astype(np.float32)
                filled_y = np.interp(idx, valid_idx, valid_y)
                filled_y = np.maximum(float(min_keep_y), filled_y - float(EDGE_REGION_FILL_PAD_PX))
                fill_mask = np.zeros_like(low_mask)
                for x, y0 in enumerate(filled_y.astype(np.int32)):
                    fill_mask[max(0, min(h - 1, int(y0))):, x] = 255
                low_mask = fill_mask
    return low_mask if cv2.countNonZero(low_mask) > 0 else None


def draw_edge_region_mask_overlay(show_frame, region_mask):
    if not EDGE_REGION_DRAW_MASK or region_mask is None or region_mask.size == 0:
        return show_frame
    mask = cv2.resize(
        region_mask,
    ...
    overlay[active] = (255, 255, 0)
    cv2.addWeighted(overlay, EDGE_REGION_DRAW_ALPHA, show_frame, 1.0 - EDGE_REGION_DRAW_ALPHA, 0, show_frame)
    cnts, _ = cv2.findContours(mask.astype(np.uint8), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    if cnts:
        cv2.drawContours(show_frame, cnts, -1, (255, 255, 0), 1)
    return show_frame
...
def build_static_bg_change_mask(gray, bg, tol, apply_cleanup=True):
    ...
    while not stop_event.is_set():
            ...
            if ENABLE_FRAME_DIFF_ROIS and frame_t1 is not None:
                with FRAME_DIFF_LOCK:
                    ...
                    if ENABLE_EDGE_REGION_LAYER:
                        ...
                        if edge_low_region_mask is not None and not EDGE_REGION_SUPPRESS_LOW:
        ...
        if draw_this_frame:
            try:
                show_frame = cv2.resize(frame, (VIDEO_STREAM_W, VIDEO_STREAM_H), interpolation=cv2.INTER_AREA)
                if ENABLE_EDGE_REGION_LAYER:
                    show_frame = draw_edge_region_mask_overlay(show_frame, edge_low_region_mask)
