...
- H265设备：`UAV_H265_DEVICES=by-id`
- 彩色输入：`UAV_H265_COLOR_CAPTURE=0`
...
- 悬停保持：`UAV_HOVER_HOLD=1`，使用方案B白天高层500米的放宽悬停确认参数
- 模型：`yolov5s_night_latest.rknn`

## 悬停确认参数

方案C夜间链路保留 H265 Y 平面、640x480 采集、160 ROI 和启动星点屏蔽，同时将悬停保持与静止确认参数同步为方案B的放宽口径：
