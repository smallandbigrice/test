...
当前口径：夜间使用 H265 彩色摄像头输入，板端硬解后只取 NV12 的 Y 平面进入方案 C；采集和帧差均为 640x480；只有帧差产生运动点时送入 YOLO；围绕运动点裁剪 160 ROI 后放大到模型输入尺寸；关闭全帧兜底；悬停保持与静止确认使用方案 B 白天高层 500 米的放宽参数；开启静止亮点屏蔽；启动后 1 分钟内检测到的小目标登记为星点区域并按检测框大小屏蔽。
