...
class VideoSender:
    def __init__(self, ip, port, width=640, height=360, quality=50):
        ...
        sndbuf = max(65536, int(os.environ.get("UAV_VIDEO_SNDBUF_BYTES", str(256 * 1024))))
        self.sock.setsockopt(socket.SOL_SOCKET, socket.SO_SNDBUF, sndbuf)
        # Video is for observation; drop frames instead of buffering stale images.
    ...
    def send(self, board_id, cam_id, frame):
        try:
            ...
            for chunk_idx in range(chunk_count):
                ...
                self._sendto(header + jpeg_data[start:end])
        except Exception:
            pass
