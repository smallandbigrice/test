...
# 1
# ==========================================
ALGORITHM_VERSION = "scheme-c-night-no-light-all-scene-h265-yplane-20260725"
ALGORITHM_NOTE = os.environ.get(
    "UAV_ALGORITHM_NOTE",
    "Frame-diff RKNN detector with optional YOLO global-local search mode.",
)
...
ENABLE_H265_YPLANE_CAPTURE = os.environ.get("UAV_H265_YPLANE_CAPTURE", "0").strip().lower() not in (
    "0",
    "false",
    "no",
)
...
GRAY_SNAPSHOT_DIR = os.environ.get("UAV_GRAY_SNAPSHOT_DIR", "").strip()
GRAY_SNAPSHOT_EVERY_SEC = max(0.0, float(os.environ.get("UAV_GRAY_SNAPSHOT_EVERY_SEC", "0")))
GRAY_RECORD_DIR = os.environ.get("UAV_GRAY_RECORD_DIR", "").strip()
GRAY_RECORD_SECONDS = max(0.0, float(os.environ.get("UAV_GRAY_RECORD_SECONDS", "0")))
GRAY_RECORD_FPS = max(1.0, float(os.environ.get("UAV_GRAY_RECORD_FPS", str(H265_YPLANE_FPS))))
GRAY_RECORD_CODEC = os.environ.get("UAV_GRAY_RECORD_CODEC", "MJPG").strip() or "MJPG"
GRAY_RECORD_MODE = os.environ.get("UAV_GRAY_RECORD_MODE", "h265_gray").strip().lower()
GRAY_RECORD_H265_BPS = max(1000000, int(os.environ.get("UAV_GRAY_RECORD_H265_BPS", "12000000")))
PIXEL_MAP_RECORD_DIR = os.environ.get("UAV_PIXEL_MAP_RECORD_DIR", "").strip()
PIXEL_MAP_RECORD_SECONDS = max(0.0, float(os.environ.get("UAV_PIXEL_MAP_RECORD_SECONDS", "0")))
PIXEL_MAP_RECORD_FPS = max(1.0, float(os.environ.get("UAV_PIXEL_MAP_RECORD_FPS", str(H265_YPLANE_FPS))))
PIXEL_MAP_RECORD_MODE = os.environ.get("UAV_PIXEL_MAP_RECORD_MODE", "h265").strip().lower()
PIXEL_MAP_RECORD_BPS = max(500000, int(os.environ.get("UAV_PIXEL_MAP_RECORD_BPS", "4000000")))
...
_EARLY_SCENE_IS_NIGHT = _EARLY_SCENE_MODE in {"night", "n", "nighttime", "dark"}
ENABLE_GRU_GRAY_SEED_ROIS = _env_bool("UAV_GRU_GRAY_SEED_ROIS", _EARLY_SCENE_IS_NIGHT)
...
ENABLE_NIGHT_STATIC_POINT_SUPPRESS = _env_bool(
    "UAV_NIGHT_STATIC_POINT_SUPPRESS",
    os.environ.get("UAV_SCENE_MODE", os.environ.get("UAV_DAY_NIGHT_MODE", "day")).strip().lower()
    in {"night", "n", "nighttime", "dark"},
)
NIGHT_STATIC_POINT_DARK_THRESH = max(0, min(255, int(os.environ.get("UAV_NIGHT_STATIC_POINT_DARK_THRESH", "85"))))
NIGHT_STATIC_POINT_BRIGHT_THRESH = max(0, min(255, int(os.environ.get("UAV_NIGHT_STATIC_POINT_BRIGHT_THRESH", "190"))))
NIGHT_STATIC_POINT_MAX_QRANGE = max(0, int(os.environ.get("UAV_NIGHT_STATIC_POINT_MAX_QRANGE", "35")))
NIGHT_STATIC_POINT_LOCAL_CONTRAST = max(0, int(os.environ.get("UAV_NIGHT_STATIC_POINT_LOCAL_CONTRAST", "18")))
NIGHT_STATIC_POINT_LOCAL_KERNEL = max(3, int(os.environ.get("UAV_NIGHT_STATIC_POINT_LOCAL_KERNEL", "21")))
if NIGHT_STATIC_POINT_LOCAL_KERNEL % 2 == 0:
    NIGHT_STATIC_POINT_LOCAL_KERNEL += 1
NIGHT_STATIC_POINT_QRANGE_OUTLIER = max(0, int(os.environ.get("UAV_NIGHT_STATIC_POINT_QRANGE_OUTLIER", "12")))
NIGHT_STATIC_POINT_MAX_AREA = max(1, int(os.environ.get("UAV_NIGHT_STATIC_POINT_MAX_AREA", "36")))
NIGHT_STATIC_POINT_MAX_SIDE = max(1, int(os.environ.get("UAV_NIGHT_STATIC_POINT_MAX_SIDE", "12")))
NIGHT_STATIC_POINT_PAD = max(0, int(os.environ.get("UAV_NIGHT_STATIC_POINT_PAD", "10")))
NIGHT_STATIC_POINT_CENTER_RADIUS = max(1, int(os.environ.get("UAV_NIGHT_STATIC_POINT_CENTER_RADIUS", "4")))
ENABLE_NIGHT_STARTUP_STAR_SUPPRESS = _env_bool("UAV_NIGHT_STARTUP_STAR_SUPPRESS", _EARLY_SCENE_IS_NIGHT)
NIGHT_STARTUP_STAR_SECONDS = max(0.0, float(os.environ.get("UAV_NIGHT_STARTUP_STAR_SECONDS", "60")))
NIGHT_STARTUP_STAR_PAD = max(0, int(os.environ.get("UAV_NIGHT_STARTUP_STAR_PAD", "6")))
NIGHT_STARTUP_STAR_MAX_BOX_W = max(1, int(os.environ.get("UAV_NIGHT_STARTUP_STAR_MAX_BOX_W", "80")))
NIGHT_STARTUP_STAR_MAX_BOX_H = max(1, int(os.environ.get("UAV_NIGHT_STARTUP_STAR_MAX_BOX_H", "80")))
NIGHT_STARTUP_STAR_MAX_BOX_AREA = max(1, int(os.environ.get("UAV_NIGHT_STARTUP_STAR_MAX_BOX_AREA", "3200")))
NIGHT_STARTUP_STAR_MIN_SCORE = min(1.0, max(0.0, float(os.environ.get("UAV_NIGHT_STARTUP_STAR_MIN_SCORE", "0.30"))))
NIGHT_STARTUP_STAR_OVERLAP_RATIO = min(1.0, max(0.0, float(os.environ.get("UAV_NIGHT_STARTUP_STAR_OVERLAP_RATIO", "0.05"))))
...
_RAW_SCENE_MODE = os.environ.get("UAV_SCENE_MODE", os.environ.get("UAV_DAY_NIGHT_MODE", "day")).strip().lower()
_RAW_SCENE_IS_NIGHT = _RAW_SCENE_MODE in {"night", "n", "nighttime", "dark"}
ENABLE_HOVER_HOLD = _env_bool("UAV_HOVER_HOLD", not _RAW_SCENE_IS_NIGHT)
...
STATIC_CONFIRM_MAX_YOLO_GAP_FRAMES = max(
    HOVER_RECHECK_INTERVAL_FRAMES,
    int(os.environ.get("UAV_STATIC_CONFIRM_MAX_YOLO_GAP_FRAMES", str(HOVER_MAX_YOLO_GAP_FRAMES))),
)
...
VIDEO_SEND_EVERY_N_FRAMES = max(
    1,
    int(os.environ.get("UAV_VIDEO_SEND_EVERY_N_FRAMES", str(DEFAULT_VIDEO_SEND_EVERY_N_FRAMES))),
)
VIDEO_STREAM_W = max(1, int(os.environ.get("UAV_VIDEO_STREAM_W", "320")))
VIDEO_STREAM_H = max(1, int(os.environ.get("UAV_VIDEO_STREAM_H", "180")))
VIDEO_STREAM_QUALITY = max(5, min(95, int(os.environ.get("UAV_VIDEO_JPEG_QUALITY", "35"))))
VIDEO_STREAM_CAM_SPECS = parse_env_list("UAV_VIDEO_STREAM_CAMS")
MAX_DRAW_BOXES = max(1, int(os.environ.get("UAV_MAX_DRAW_BOXES", "32")))
...
ENABLE_EDGE_REGION_LAYER = _env_bool(
    "UAV_EDGE_REGION_LAYER",
    SCENE_MODE == "day" and not SCENE_IS_NIGHT and LAYER_MODE == "auto",
)
EDGE_REGION_UPDATE_FRAMES = max(1, int(os.environ.get("UAV_EDGE_REGION_UPDATE_FRAMES", "30")))
EDGE_REGION_CANNY_LOW = max(1, int(os.environ.get("UAV_EDGE_REGION_CANNY_LOW", "45")))
EDGE_REGION_CANNY_HIGH = max(EDGE_REGION_CANNY_LOW + 1, int(os.environ.get("UAV_EDGE_REGION_CANNY_HIGH", "120")))
EDGE_REGION_DENSITY_KERNEL = max(7, int(os.environ.get("UAV_EDGE_REGION_DENSITY_KERNEL", "31")))
if EDGE_REGION_DENSITY_KERNEL % 2 == 0:
    EDGE_REGION_DENSITY_KERNEL += 1
...
if EDGE_REGION_CLOSE_SIZE % 2 == 0:
    EDGE_REGION_CLOSE_SIZE += 1
EDGE_REGION_DILATE_SIZE = max(3, int(os.environ.get("UAV_EDGE_REGION_DILATE_SIZE", "17")))
if EDGE_REGION_DILATE_SIZE % 2 == 0:
...
STATIC_CONFIRM_MAX_AGE_FRAMES = max(
    TRACK_SEARCH_MAX_AGE_FRAMES,
    int(os.environ.get("UAV_STATIC_CONFIRM_MAX_AGE_FRAMES", str(STATIC_CONFIRM_MAX_YOLO_GAP_FRAMES))),
)
...
ENABLE_LOW_MEDIAN_BG_GATE = os.environ.get("UAV_LOW_MEDIAN_BG_GATE", "1").strip().lower() not in (
    "0",
    "false",
    "no",
)
LOW_MEDIAN_BG_BASE_DELTA = float(os.environ.get("UAV_LOW_MEDIAN_BG_BASE_DELTA", "12.0"))
LOW_MEDIAN_BG_QRANGE_MULT = float(os.environ.get("UAV_LOW_MEDIAN_BG_QRANGE_MULT", "2.8"))
LOW_MEDIAN_BG_MAX_DELTA = float(os.environ.get("UAV_LOW_MEDIAN_BG_MAX_DELTA", "150.0"))
LOW_MEDIAN_BG_UNSTABLE_QRANGE = float(os.environ.get("UAV_LOW_MEDIAN_BG_UNSTABLE_QRANGE", "14.0"))
LOW_MEDIAN_BG_UNSTABLE_EXTRA_DELTA = float(os.environ.get("UAV_LOW_MEDIAN_BG_UNSTABLE_EXTRA_DELTA", "12.0"))
ENABLE_LOW_BG_DENSE_SUPPRESS = os.environ.get("UAV_LOW_BG_DENSE_SUPPRESS", "1").strip().lower() not in (
    "0",
    "false",
    "no",
)
LOW_BG_DENSE_KERNEL = max(9, int(os.environ.get("UAV_LOW_BG_DENSE_KERNEL", "81")))
if LOW_BG_DENSE_KERNEL % 2 == 0:
...
VIDEO_FORCE_LOW_BG_30 = _env_bool("UAV_VIDEO_FORCE_LOW_BG_30", True)
DEFAULT_ONLINE_AUTO_BG_SECONDS = 30.0 if (SCENE_MODE == "day" and LAYER_MODE == "auto") else 0.0
ONLINE_AUTO_BG_SECONDS = max(
    0.0,
    float(os.environ.get("UAV_ONLINE_AUTO_BG_SECONDS", str(DEFAULT_ONLINE_AUTO_BG_SECONDS))),
)
VIBE_TILE_MODE = _env_bool("UAV_VIBE_TILE_ROIS", False)
YOLO_GLOBAL_LOCAL_MODE = _env_bool("UAV_YOLO_GLOBAL_LOCAL_MODE", False)
if VIBE_TILE_MODE:
    YOLO_GLOBAL_LOCAL_MODE = False
ENABLE_FRAME_DIFF_ROIS = not YOLO_GLOBAL_LOCAL_MODE
DIRECT_FULL_FRAME_INFERENCE = _env_bool("UAV_DIRECT_FULL_FRAME_INFERENCE", DEFAULT_DIRECT_FULL_FRAME)
if YOLO_GLOBAL_LOCAL_MODE:
    DIRECT_FULL_FRAME_INFERENCE = False
if VIBE_TILE_MODE:
    DIRECT_FULL_FRAME_INFERENCE = False
...
YOLO_GLOBAL_SCAN_INTERVAL_FRAMES = max(
    1,
    int(os.environ.get("UAV_YOLO_GLOBAL_SCAN_INTERVAL_FRAMES", "15")),
)
YOLO_GLOBAL_SCAN_SCORE = float(os.environ.get("UAV_YOLO_GLOBAL_SCAN_SCORE", "1.0"))
YOLO_LOCAL_ROI_MAX = max(1, int(os.environ.get("UAV_YOLO_LOCAL_ROI_MAX", "4")))
YOLO_LOCAL_ROI_CROP_SIZE = max(64, int(os.environ.get("UAV_YOLO_LOCAL_ROI_SIZE", str(CROP_SIZE))))
YOLO_LOCAL_ROI_MAX_AGE_FRAMES = max(
    1,
    int(os.environ.get("UAV_YOLO_LOCAL_ROI_MAX_AGE_FRAMES", str(PROCESS_EVERY_N_FRAMES * 8))),
)
YOLO_LOCAL_ROI_PREDICT_MAX_MISSES = max(
    0,
    int(os.environ.get("UAV_YOLO_LOCAL_ROI_PREDICT_MAX_MISSES", "8")),
)
YOLO_LOCAL_ROI_MIN_YOLO_HITS = max(1, int(os.environ.get("UAV_YOLO_LOCAL_ROI_MIN_HITS", "1")))
YOLO_LOCAL_ROI_MIN_SCORE = float(os.environ.get("UAV_YOLO_LOCAL_ROI_MIN_SCORE", "0.16"))
VIBE_TILE_W = max(64, int(os.environ.get("UAV_VIBE_TILE_W", "640")))
VIBE_TILE_H = max(64, int(os.environ.get("UAV_VIBE_TILE_H", "480")))
VIBE_TILE_STEP_X = max(32, int(os.environ.get("UAV_VIBE_TILE_STEP_X", str(VIBE_TILE_W))))
VIBE_TILE_STEP_Y = max(32, int(os.environ.get("UAV_VIBE_TILE_STEP_Y", str(VIBE_TILE_H))))
VIBE_TILE_MAX_ROIS = max(1, int(os.environ.get("UAV_VIBE_TILE_MAX_ROIS", "4")))
VIBE_TILE_KEEP_TRACK_ROIS = _env_bool("UAV_VIBE_TILE_KEEP_TRACK_ROIS", True)
VIBE_SAMPLES = max(3, int(os.environ.get("UAV_VIBE_SAMPLES", "8")))
VIBE_RADIUS = max(1, int(os.environ.get("UAV_VIBE_RADIUS", "18")))
VIBE_MIN_MATCHES = max(1, int(os.environ.get("UAV_VIBE_MIN_MATCHES", "2")))
VIBE_UPDATE_FACTOR = max(2, int(os.environ.get("UAV_VIBE_UPDATE_FACTOR", "16")))
VIBE_WARMUP_FRAMES = max(1, int(os.environ.get("UAV_VIBE_WARMUP_FRAMES", "3")))
VIBE_OPEN_ITER = max(0, int(os.environ.get("UAV_VIBE_OPEN_ITER", "1")))
VIBE_DILATE_ITER = max(0, int(os.environ.get("UAV_VIBE_DILATE_ITER", "1")))
VIBE_MIN_TILE_FG_PIXELS = max(1, int(os.environ.get("UAV_VIBE_MIN_TILE_FG_PIXELS", "18")))
VIBE_MIN_COMPONENT_AREA = max(1, int(os.environ.get("UAV_VIBE_MIN_COMPONENT_AREA", "8")))
VIBE_MAX_COMPONENT_AREA = max(
    VIBE_MIN_COMPONENT_AREA,
    int(os.environ.get("UAV_VIBE_MAX_COMPONENT_AREA", "5000")),
)
VIBE_MAX_TILE_FG_RATIO = max(0.001, float(os.environ.get("UAV_VIBE_MAX_TILE_FG_RATIO", "0.12")))
...
MOTION_CLOSE_ITER = 1
LOW_LAYER_MOTION_ERODE_ITER = max(0, int(os.environ.get("UAV_LOW_MOTION_ERODE_ITER", str(MOTION_ERODE_ITER))))
LOW_LAYER_MOTION_DILATE_ITER = max(0, int(os.environ.get("UAV_LOW_MOTION_DILATE_ITER", str(MOTION_DILATE_ITER))))
LOW_LAYER_MOTION_CLOSE_ITER = max(0, int(os.environ.get("UAV_LOW_MOTION_CLOSE_ITER", str(MOTION_CLOSE_ITER))))
...
ROI_GRID_MAX_PER_CELL = max(1, int(os.environ.get("UAV_ROI_GRID_MAX_PER_CELL", "2")))
...
LOW_LAYER_MIN_DIFF_AREA = max(1, int(os.environ.get("UAV_LOW_MIN_DIFF_AREA", "15")))
...
def make_layer_profile(high, reason="global", day_scene=None, layer_confidence=0.0):
    ...
    return {
        ...
        "motion_erode_iter": MOTION_ERODE_ITER if high else LOW_LAYER_MOTION_ERODE_ITER,
        "motion_dilate_iter": MOTION_DILATE_ITER if high else LOW_LAYER_MOTION_DILATE_ITER,
        "motion_close_iter": MOTION_CLOSE_ITER if high else LOW_LAYER_MOTION_CLOSE_ITER,
...
def online_static_bg_seconds(profile):
    seconds = float(profile.get("static_bg_seconds", STATIC_BG_SECONDS))
    if SCENE_MODE == "day" and LAYER_MODE == "auto" and ENABLE_AUTO_CAM_LAYER:
        seconds = max(seconds, ONLINE_AUTO_BG_SECONDS)
...
def roi_layer_name(roi, default_name=None):
    if roi and isinstance(roi[-1], str) and roi[-1] in ("high", "low"):
        return roi[-1]
    return default_name or current_layer_profile().get("name", "high")


def tag_roi_layer(roi, layer_name):
    if roi and isinstance(roi[-1], str) and roi[-1] in ("high", "low"):
        return roi
    return tuple(list(roi) + [str(layer_name)])


def add_roi_score(roi, delta):
    if not roi or len(roi) <= 4:
        return roi
    vals = list(roi)
    vals[4] = float(vals[4]) + float(delta)
    return tuple(vals)


def roi_has_region_support(roi, region_mask, full_w, full_h):
    if region_mask is None or region_mask.size == 0 or cv2.countNonZero(region_mask) <= 0:
        return False
    mh, mw = region_mask.shape[:2]
    sx = mw / float(max(1, full_w))
    sy = mh / float(max(1, full_h))
    seed_x, seed_y = roi_seed_center(roi)
    radius_x = max(1, int(round(EDGE_REGION_ROI_SUPPORT_RADIUS * sx)))
    radius_y = max(1, int(round(EDGE_REGION_ROI_SUPPORT_RADIUS * sy)))
    mx = int(round(seed_x * sx))
    my = int(round(seed_y * sy))
...
def edge_region_profiles():
    high_profile = make_layer_profile(True, "edge-region:sky-high", day_scene=SCENE_MODE == "day", layer_confidence=1.0)
    low_profile = make_layer_profile(False, "edge-region:edge-low", day_scene=SCENE_MODE == "day", layer_confidence=1.0)
    return high_profile, low_profile


def build_edge_low_region_mask(gray, bg_gray=None):
    if not ENABLE_EDGE_REGION_LAYER:
        return None
    src = bg_gray if bg_gray is not None and bg_gray.shape == gray.shape else gray
    if src is None or src.size == 0:
        return None
    h, _w = src.shape[:2]
    blur = cv2.GaussianBlur(src, (5, 5), 0)
    edges = cv2.Canny(blur, EDGE_REGION_CANNY_LOW, EDGE_REGION_CANNY_HIGH)
    density = cv2.blur(
        (edges > 0).astype(np.float32),
        (EDGE_REGION_DENSITY_KERNEL, EDGE_REGION_DENSITY_KERNEL),
    )
    dense = (density >= EDGE_REGION_DENSITY_THRESH).astype(np.uint8) * 255
    dense = cv2.morphologyEx(dense, cv2.MORPH_CLOSE, EDGE_REGION_CLOSE_KERNEL, iterations=1)
    dense = cv2.dilate(dense, EDGE_REGION_DILATE_KERNEL, iterations=1)
    cnts, _ = cv2.findContours(dense, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    low_mask = np.zeros_like(dense)
    min_keep_y = int(max(0.0, min(0.95, EDGE_REGION_KEEP_Y_RATIO)) * h)
    for c in cnts:
        if int(cv2.contourArea(c)) < EDGE_REGION_MIN_AREA:
            continue
        _x, y, _cw, ch = cv2.boundingRect(c)
        if y + ch < min_keep_y:
            continue
        cv2.drawContours(low_mask, [c], -1, 255, thickness=cv2.FILLED)
    return low_mask if cv2.countNonZero(low_mask) > 0 else None
...
def make_cam_profile_from_background(cam_idx, bg_gray, bg_qrange):
    ...
    if spec in ("high", "h"):
        high, reason = True, f"debug-override:high; {auto_reason}"
    elif spec in ("low", "l"):
        high, reason = False, f"debug-override:low; {auto_reason}"
    elif ENABLE_EDGE_REGION_LAYER:
        high, reason = True, f"edge-region:base-high; {auto_reason}"
    elif spec in ("auto", "a") or ENABLE_AUTO_CAM_LAYER:
        high, reason = auto_high, auto_reason
    else:
        high, reason = HIGH_LAYER_MODE, f"debug-global:{LAYER_MODE}; {auto_reason}"
    return make_layer_profile(
        high,
        reason,
        day_scene=day_scene,
...
def build_cam_layer_profiles(initial_backgrounds):
    ...
    for cam_idx in range(N_CAM):
        ...
        profiles.append(make_cam_profile_from_background(cam_idx, bg_gray, bg_qrange))
    return profiles
...
def box_overlap_metrics(a, b):
    ...
    iou = inter / union
    cover = inter / max(1.0, min(area_a, area_b))
    return iou, cover

def suppress_overlapping_rois(
    rois,
    iou_thresh=ROI_OVERLAP_IOU_THRESH,
    cover_thresh=ROI_OVERLAP_COVER_THRESH,
):
    if not ENABLE_ROI_OVERLAP_SUPPRESS or not rois:
        return rois
    selected = []
    sorted_rois = sorted(rois, key=lambda r: r[4] if len(r) > 4 else 0.0, reverse=True)
    for roi in sorted_rois:
        key = roi_suppress_key(roi)
        duplicate = False
        for kept in selected:
            if key != roi_suppress_key(kept):
                continue
            iou, cover = box_overlap_metrics(roi, kept)
            if iou >= iou_thresh or cover >= cover_thresh:
                duplicate = True
                break
        if not duplicate:
            selected.append(roi)
...
def night_static_point_suppress_enabled(profile):
    return bool(
        ENABLE_NIGHT_STATIC_POINT_SUPPRESS
        and SCENE_IS_NIGHT
        and (profile or {}).get("high", False)
    )


def build_night_static_point_mask(bg_gray, bg_qrange, profile):
    if not night_static_point_suppress_enabled(profile) or bg_gray is None:
        return None
    if len(bg_gray.shape) != 2:
        bg_gray = frame_to_gray(bg_gray)
    if bg_gray is None or bg_gray.size == 0:
        return None

    dark = bg_gray <= NIGHT_STATIC_POINT_DARK_THRESH
    bright = bg_gray >= NIGHT_STATIC_POINT_BRIGHT_THRESH
    candidate = np.logical_or(dark, bright)
    local_delta = None
    if NIGHT_STATIC_POINT_LOCAL_CONTRAST > 0:
        local_bg = cv2.medianBlur(bg_gray, NIGHT_STATIC_POINT_LOCAL_KERNEL)
        local_delta = cv2.absdiff(bg_gray, local_bg)
        candidate = np.logical_or(candidate, local_delta >= NIGHT_STATIC_POINT_LOCAL_CONTRAST)
    qrange_outlier = None
    if bg_qrange is not None:
        if bg_qrange.shape[:2] != bg_gray.shape[:2]:
            bg_qrange = cv2.resize(bg_qrange, (bg_gray.shape[1], bg_gray.shape[0]), interpolation=cv2.INTER_NEAREST)
        if NIGHT_STATIC_POINT_QRANGE_OUTLIER > 0:
            q_local = cv2.medianBlur(bg_qrange, NIGHT_STATIC_POINT_LOCAL_KERNEL)
            q_delta = cv2.subtract(bg_qrange, q_local)
            qrange_outlier = q_delta >= NIGHT_STATIC_POINT_QRANGE_OUTLIER
            candidate = np.logical_or(candidate, qrange_outlier)
        stable = bg_qrange <= NIGHT_STATIC_POINT_MAX_QRANGE
        if local_delta is not None and NIGHT_STATIC_POINT_LOCAL_CONTRAST > 0:
            strong_local = local_delta >= max(
                NIGHT_STATIC_POINT_LOCAL_CONTRAST + 8,
                int(round(NIGHT_STATIC_POINT_LOCAL_CONTRAST * 1.5)),
            )
            stable = np.logical_or(stable, strong_local)
        if qrange_outlier is not None:
            stable = np.logical_or(stable, qrange_outlier)
        candidate = np.logical_and(candidate, stable)

    base_mask = candidate.astype(np.uint8) * 255
    cnts, _ = cv2.findContours(base_mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    point_mask = np.zeros(bg_gray.shape[:2], dtype=np.uint8)
    for c in cnts:
        x, y, w, h = cv2.boundingRect(c)
        if w <= 0 or h <= 0:
            continue
        area = int(cv2.countNonZero(base_mask[y:y + h, x:x + w]))
        if area > NIGHT_STATIC_POINT_MAX_AREA:
            continue
        if max(w, h) > NIGHT_STATIC_POINT_MAX_SIDE:
            continue
        cv2.drawContours(point_mask, [c], -1, 255, thickness=-1)

    if NIGHT_STATIC_POINT_PAD > 0 and cv2.countNonZero(point_mask) > 0:
        k = 2 * NIGHT_STATIC_POINT_PAD + 1
        kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (k, k))
        point_mask = cv2.dilate(point_mask, kernel, iterations=1)
    return point_mask if cv2.countNonZero(point_mask) > 0 else None


def night_static_point_hit(mask, cx, cy, full_w, full_h, radius=NIGHT_STATIC_POINT_CENTER_RADIUS):
    if mask is None or full_w <= 0 or full_h <= 0:
        return False
    mh, mw = mask.shape[:2]
    mx = int(round(float(cx) * mw / float(full_w)))
    my = int(round(float(cy) * mh / float(full_h)))
    mx1 = max(0, mx - radius)
    my1 = max(0, my - radius)
    mx2 = min(mw, mx + radius + 1)
    my2 = min(mh, my + radius + 1)
    if mx2 <= mx1 or my2 <= my1:
        return False
    return cv2.countNonZero(mask[my1:my2, mx1:mx2]) > 0


def filter_night_static_point_rois(rois, point_mask, full_w, full_h, profile):
    if not night_static_point_suppress_enabled(profile) or point_mask is None or not rois:
        return rois
    kept = []
    for roi in rois:
        cx, cy = roi_seed_center(roi)
        if night_static_point_hit(point_mask, cx, cy, full_w, full_h):
            continue
        kept.append(roi)
    return kept


def apply_night_static_point_mask(motion_mask, point_mask, profile):
    if not night_static_point_suppress_enabled(profile) or motion_mask is None or point_mask is None:
        return motion_mask
    if point_mask.shape[:2] != motion_mask.shape[:2]:
        point_mask = cv2.resize(
            point_mask,
            (motion_mask.shape[1], motion_mask.shape[0]),
            interpolation=cv2.INTER_NEAREST,
        )
    return cv2.bitwise_and(motion_mask, cv2.bitwise_not(point_mask))


def is_night_static_point_box(box, point_mask, full_w, full_h, profile):
    if not night_static_point_suppress_enabled(profile) or point_mask is None or box is None or len(box) < 4:
        return False
    cx = (float(box[0]) + float(box[2])) * 0.5
    cy = (float(box[1]) + float(box[3])) * 0.5
    return night_static_point_hit(point_mask, cx, cy, full_w, full_h)


def startup_star_suppress_enabled(profile):
    return bool(
        ENABLE_NIGHT_STARTUP_STAR_SUPPRESS
        and SCENE_IS_NIGHT
        and NIGHT_STARTUP_STAR_SECONDS > 0.0
        and (profile or {}).get("high", False)
    )


def startup_star_learning_active(start_ts):
    return bool(
        ENABLE_NIGHT_STARTUP_STAR_SUPPRESS
        and NIGHT_STARTUP_STAR_SECONDS > 0.0
        and (time.monotonic() - float(start_ts)) <= NIGHT_STARTUP_STAR_SECONDS
    )


def startup_star_candidate_box(box):
    if box is None or len(box) < 4:
        return False
    x1, y1, x2, y2 = [float(v) for v in box[:4]]
    w = max(0.0, x2 - x1)
    ...
    return bool(
        w > 0.0
        and h > 0.0
        and w <= NIGHT_STARTUP_STAR_MAX_BOX_W
        and h <= NIGHT_STARTUP_STAR_MAX_BOX_H
        and area <= NIGHT_STARTUP_STAR_MAX_BOX_AREA
        and score >= NIGHT_STARTUP_STAR_MIN_SCORE
    )


def add_startup_star_box(mask, box, full_w, full_h):
    if mask is None or not startup_star_candidate_box(box) or full_w <= 0 or full_h <= 0:
        return False
    mh, mw = mask.shape[:2]
    sx = mw / float(full_w)
    sy = mh / float(full_h)
    x1 = max(0, int(math.floor(float(box[0]) * sx)) - NIGHT_STARTUP_STAR_PAD)
    y1 = max(0, int(math.floor(float(box[1]) * sy)) - NIGHT_STARTUP_STAR_PAD)
    x2 = min(mw, int(math.ceil(float(box[2]) * sx)) + NIGHT_STARTUP_STAR_PAD)
    y2 = min(mh, int(math.ceil(float(box[3]) * sy)) + NIGHT_STARTUP_STAR_PAD)
    if x2 <= x1 or y2 <= y1:
        return False
    before = cv2.countNonZero(mask[y1:y2, x1:x2])
    mask[y1:y2, x1:x2] = 255
    after = cv2.countNonZero(mask[y1:y2, x1:x2])
    return after > before


def startup_star_box_hit(mask, box, full_w, full_h):
    if mask is None or box is None or len(box) < 4 or full_w <= 0 or full_h <= 0:
        return False
    mh, mw = mask.shape[:2]
    sx = mw / float(full_w)
    sy = mh / float(full_h)
    x1 = max(0, int(math.floor(float(box[0]) * sx)))
    y1 = max(0, int(math.floor(float(box[1]) * sy)))
    x2 = min(mw, int(math.ceil(float(box[2]) * sx)))
    y2 = min(mh, int(math.ceil(float(box[3]) * sy)))
    if x2 <= x1 or y2 <= y1:
        return False
    roi_mask = mask[y1:y2, x1:x2]
    hit = cv2.countNonZero(roi_mask)
    if hit <= 0:
        return False
    box_area = max(1, (x2 - x1) * (y2 - y1))
    return (float(hit) / float(box_area)) >= NIGHT_STARTUP_STAR_OVERLAP_RATIO


def filter_startup_star_rois(rois, star_mask, full_w, full_h):
    if star_mask is None or not rois:
        return rois
    kept = []
    for roi in rois:
        if startup_star_box_hit(star_mask, roi, full_w, full_h):
            continue
        kept.append(roi)
    return kept


def apply_startup_star_mask(motion_mask, star_mask):
    if motion_mask is None or star_mask is None:
        return motion_mask
    if star_mask.shape[:2] != motion_mask.shape[:2]:
        star_mask = cv2.resize(
            star_mask,
            (motion_mask.shape[1], motion_mask.shape[0]),
            interpolation=cv2.INTER_NEAREST,
        )
    return cv2.bitwise_and(motion_mask, cv2.bitwise_not(star_mask))
...
def build_static_bg_change_mask(gray, bg, tol, bg_qrange=None, profile=None, apply_cleanup=True):
    if not ENABLE_LOW_LAYER_STATIC_BG_MASK or bg is None or tol is None:
        return None
    profile = profile or current_layer_profile()
    bg_diff = cv2.absdiff(gray, bg)
    tol_map = tol.astype(np.float32)
    qrange_map = None
    if (
        profile.get("enable_median_bg_gate", False)
        and bg_qrange is not None
        and getattr(bg_qrange, "ndim", 0) == 2
        and bg_qrange.size > 0
    ):
        qrange_map = bg_qrange
        if qrange_map.shape[:2] != gray.shape[:2]:
            qrange_map = cv2.resize(qrange_map, (gray.shape[1], gray.shape[0]), interpolation=cv2.INTER_NEAREST)
        q = qrange_map.astype(np.float32)
        median_tol = LOW_MEDIAN_BG_BASE_DELTA + LOW_MEDIAN_BG_QRANGE_MULT * q
        median_tol += (q >= LOW_MEDIAN_BG_UNSTABLE_QRANGE).astype(np.float32) * LOW_MEDIAN_BG_UNSTABLE_EXTRA_DELTA
        median_tol = np.clip(median_tol, LOW_MEDIAN_BG_BASE_DELTA, LOW_MEDIAN_BG_MAX_DELTA)
        tol_map = np.maximum(tol_map, median_tol)

    changed = (bg_diff.astype(np.float32) > tol_map).astype(np.uint8) * 255
    if (
        profile.get("enable_median_bg_gate", False)
        and ENABLE_LOW_BG_DENSE_SUPPRESS
        and qrange_map is not None
        and cv2.countNonZero(changed) > 0
    ):
        active = (changed > 0).astype(np.float32)
        density = cv2.blur(active, (LOW_BG_DENSE_KERNEL, LOW_BG_DENSE_KERNEL))
        noisy_region = np.logical_and(
            density >= LOW_BG_DENSE_RATIO,
            qrange_map.astype(np.float32) >= LOW_BG_DENSE_MIN_QRANGE,
        )
        if np.any(noisy_region):
            changed[noisy_region] = 0
    if apply_cleanup:
...
def make_padded_roi_canvas(frame, roi, crop_size=CROP_SIZE, fill_value=TIGHT_MOTION_ROI_FILL):
    ...
    if len(patch.shape) == 2:
        canvas = np.full((crop_size, crop_size), int(fill_value), dtype=np.uint8)
    else:
        canvas = np.full((crop_size, crop_size, patch.shape[2]), int(fill_value), dtype=np.uint8)
...
def pad_roi_image_to_square(roi_img, crop_size=CROP_SIZE, fill_value=TIGHT_MOTION_ROI_FILL):
    ...
    if len(roi_img.shape) == 2:
        ...
        canvas = np.full((crop_size, crop_size, roi_img.shape[2]), int(fill_value), dtype=np.uint8)
...
class VibeTileDetector:
    ...
    def _init_samples(self, gray):
        gray = np.asarray(gray, dtype=np.uint8)
        self.samples = np.empty((VIBE_SAMPLES, gray.shape[0], gray.shape[1]), dtype=np.uint8)
        for i in range(VIBE_SAMPLES):
            dx = int(self.rng.integers(-1, 2))
            dy = int(self.rng.integers(-1, 2))
            shifted = np.roll(gray, shift=(dy, dx), axis=(0, 1))
            noise = self.rng.integers(-2, 3, size=gray.shape, dtype=np.int16)
            self.samples[i] = np.clip(shifted.astype(np.int16) + noise, 0, 255).astype(np.uint8)
        self.frame_count = 0

    def _foreground_mask(self, gray):
        diff = np.abs(self.samples.astype(np.int16) - gray.astype(np.int16)[None, :, :])
        matches = np.count_nonzero(diff <= VIBE_RADIUS, axis=0)
        foreground = (matches < VIBE_MIN_MATCHES).astype(np.uint8) * 255
        if VIBE_OPEN_ITER > 0:
            foreground = cv2.morphologyEx(
                foreground,
                cv2.MORPH_OPEN,
                self.open_kernel,
                iterations=VIBE_OPEN_ITER,
            )
        if VIBE_DILATE_ITER > 0:
            foreground = cv2.dilate(foreground, self.dilate_kernel, iterations=VIBE_DILATE_ITER)
        background = matches >= VIBE_MIN_MATCHES
        return foreground, background

    def _update_samples(self, gray, background):
        update_mask = background & (self.rng.integers(0, VIBE_UPDATE_FACTOR, size=gray.shape) == 0)
        ys, xs = np.where(update_mask)
        if len(xs) == 0:
            return
        sample_ids = self.rng.integers(0, VIBE_SAMPLES, size=len(xs))
        self.samples[sample_ids, ys, xs] = gray[ys, xs]

    def _tile_origins(self, full_w, full_h):
        xs = list(range(0, max(1, full_w - VIBE_TILE_W + 1), VIBE_TILE_STEP_X))
        ys = list(range(0, max(1, full_h - VIBE_TILE_H + 1), VIBE_TILE_STEP_Y))
        if not xs or xs[-1] != max(0, full_w - VIBE_TILE_W):
            xs.append(max(0, full_w - VIBE_TILE_W))
        if not ys or ys[-1] != max(0, full_h - VIBE_TILE_H):
            ys.append(max(0, full_h - VIBE_TILE_H))
        return sorted(set(xs)), sorted(set(ys))

    def _score_tile(self, mask_patch):
        fg_pixels = int(cv2.countNonZero(mask_patch))
        if fg_pixels < VIBE_MIN_TILE_FG_PIXELS:
            return None
        fg_ratio = fg_pixels / float(max(1, mask_patch.size))
        if fg_ratio > VIBE_MAX_TILE_FG_RATIO:
            return None
        num_labels, _labels, stats, _centroids = cv2.connectedComponentsWithStats(mask_patch, 8)
        component_areas = []
        for label_idx in range(1, num_labels):
            area = int(stats[label_idx, cv2.CC_STAT_AREA])
            if VIBE_MIN_COMPONENT_AREA <= area <= VIBE_MAX_COMPONENT_AREA:
                component_areas.append(area)
        if not component_areas:
            return None
        largest = max(component_areas)
        compact_components = len(component_areas)
        return 100.0 + float(largest) + 8.0 * float(compact_components) - 200.0 * fg_ratio

    def update_and_get_rois(self, gray, full_w, full_h, scale_x, scale_y):
        if gray is None or gray.size == 0:
            return [], None
        gray = np.asarray(gray, dtype=np.uint8)
        if self.samples is None or self.samples.shape[1:] != gray.shape[:2]:
            self._init_samples(gray)
            return [], None
        foreground, background = self._foreground_mask(gray)
        self._update_samples(gray, background)
        self.frame_count += 1
        if self.frame_count <= VIBE_WARMUP_FRAMES:
            return [], foreground

        rois = []
        xs, ys = self._tile_origins(int(full_w), int(full_h))
        for y1 in ys:
            for x1 in xs:
                x2 = min(int(full_w), x1 + VIBE_TILE_W)
                y2 = min(int(full_h), y1 + VIBE_TILE_H)
                mx1 = max(0, min(foreground.shape[1] - 1, int(math.floor(x1 / max(1e-6, scale_x)))))
                my1 = max(0, min(foreground.shape[0] - 1, int(math.floor(y1 / max(1e-6, scale_y)))))
                mx2 = max(mx1 + 1, min(foreground.shape[1], int(math.ceil(x2 / max(1e-6, scale_x)))))
                my2 = max(my1 + 1, min(foreground.shape[0], int(math.ceil(y2 / max(1e-6, scale_y)))))
                score = self._score_tile(foreground[my1:my2, mx1:mx2])
                if score is None:
                    continue
                rois.append((x1, y1, x2, y2, score))
        rois.sort(key=lambda r: r[4], reverse=True)
        return rois[:VIBE_TILE_MAX_ROIS], foreground
...
class TrajectoryFilter:
    ...
    def _is_hover_candidate(self, trk, frame_idx):
        ...
        if float(trk.get('score', 0.0)) < HOVER_MIN_TRACK_SCORE:
            return False
        if not self._is_confirmed(trk, frame_idx=frame_idx):
            return False
    ...
    def _is_static_confirmed(self, trk, frame_idx=None):
        if not ENABLE_STATIC_CONFIRM:
            return False
        if trk.get('yolo_hits', 0) < STATIC_CONFIRM_YOLO_HITS:
            return False
        recent_window = min(self.recent_window, max(STATIC_CONFIRM_RECENT_WINDOW, STATIC_CONFIRM_RECENT_HITS))
        if self._recent_yolo_hits(trk, recent_window) < STATIC_CONFIRM_RECENT_HITS:
            return False
        if trk.get('misses', 0) > STATIC_CONFIRM_KEEP_MISSES:
            return False
        if frame_idx is not None and self._visual_lock_yolo_gap(trk, frame_idx) > STATIC_CONFIRM_MAX_YOLO_GAP_FRAMES:
            return False
    ...
    def _is_confirmed(self, trk, frame_idx=None):
        if self._is_static_confirmed(trk, frame_idx=frame_idx):
            return True
    ...
    def update(self, detections, frame=None, frame_idx=0):
        ...
        for trk in self.trackers:
            max_misses = STATIC_CONFIRM_KEEP_MISSES if self._is_static_confirmed(trk) else YOLO_TRACK_MAX_SEARCH_MISSES
            min_score = 0.08 if max_misses > YOLO_TRACK_MAX_SEARCH_MISSES else 0.12
            if trk['misses'] <= max_misses and trk['score'] >= min_score:
                kept.append(trk)
        self.trackers = kept
        ...
        for trk in self.trackers:
            ...
            if recheck_min_gap_frames > 0:
                ...
                if int(frame_idx) - last_yolo_frame < recheck_min_gap_frames:
                    continue
            if self.profile["track_search_confirmed_only"] and not self._is_confirmed(trk, frame_idx=frame_idx):
                continue
    ...
    def get_yolo_global_local_rois(self, frame_idx, full_w, full_h, max_rois=YOLO_LOCAL_ROI_MAX):
        rois = []
        max_age_frames = int(YOLO_LOCAL_ROI_MAX_AGE_FRAMES)
        max_predict_misses = int(YOLO_LOCAL_ROI_PREDICT_MAX_MISSES)
        max_rois = int(max_rois)
        for trk in self.trackers:
            if int(trk.get('yolo_hits', 0)) < YOLO_LOCAL_ROI_MIN_YOLO_HITS:
                continue
            if int(trk.get('misses', 0)) > YOLO_TRACK_MAX_SEARCH_MISSES:
                continue
            track_age = max(0, int(frame_idx) - int(trk.get('last_frame', frame_idx)))
            if track_age > max_age_frames:
                continue
            if float(trk.get('score', 0.0)) < YOLO_LOCAL_ROI_MIN_SCORE:
                continue
            box = (
                self._predicted_box(trk, frame_idx)
                if 0 < int(trk.get('misses', 0)) <= max_predict_misses
                else trk['box']
            )
            cx = (float(box[0]) + float(box[2])) * 0.5
            cy = (float(box[1]) + float(box[3])) * 0.5
            crop_size = min(max(64, int(YOLO_LOCAL_ROI_CROP_SIZE)), max(int(full_w), int(full_h)))
            rx1, ry1, rx2, ry2 = crop_roi_from_center(cx, cy, full_w, full_h, crop_size=crop_size)
            priority = 1500.0 + float(trk.get('score', 0.0)) - 10.0 * float(trk.get('misses', 0))
            rois.append((rx1, ry1, rx2, ry2, priority))
        rois = merge_nearby_boxes(rois, dist_thresh=160)
        rois.sort(key=lambda r: r[4] if len(r) > 4 else 0.0, reverse=True)
        return rois[:max_rois]
    ...
    def get_confirmed_tracks(self, frame_idx=0):
        ...
        for trk in self.trackers:
            track_age = max(0, int(frame_idx) - int(trk.get('last_frame', frame_idx)))
            static_confirmed = self._is_static_confirmed(trk, frame_idx=frame_idx)
            max_age_frames = STATIC_CONFIRM_MAX_AGE_FRAMES if static_confirmed else TRACK_SEARCH_MAX_AGE_FRAMES
            if track_age > max_age_frames:
                continue
            if not static_confirmed and not self._is_confirmed(trk, frame_idx=frame_idx):
                continue
            box = trk['box']
            ...
            confirmed.append({
                'id': trk['id'],
                'box': box,
                'layer': trk.get('layer', self.profile.get("name", "high")),
                'history': history,
                'score': trk['score'],
                ...
                'static_confirmed': bool(static_confirmed),
                'last_yolo_frame': int(trk.get('last_yolo_frame', trk.get('last_frame', frame_idx))),
            })
        return confirmed
...
_H265_DEVICE_CACHE = None


def _video_node_sort_key(path):
    m = re.search(r"video(\d+)$", str(path))
...
def get_h265_device_for_cam(cam_idx):
    if not ENABLE_H265_YPLANE_CAPTURE:
        return None
    if H265_YPLANE_DEVICES:
        return H265_YPLANE_DEVICES[cam_idx] if cam_idx < len(H265_YPLANE_DEVICES) else None
...
class H265YPlaneCapture:
    ...
    def _open(self):
        try:
            ...
            raise RuntimeError(f"Python GStreamer unavailable for H265 Y-plane capture: {exc}") from exc
        ...
        if self.sink is None:
            raise RuntimeError("H265 Y-plane appsink not found")
        self.sink.set_property("emit-signals", False)
        self.sink.set_property("sync", False)
        self.sink.set_property("drop", True)
        self.sink.set_property("max-buffers", 1)
        status = self.pipeline.set_state(Gst.State.PLAYING)
        self.opened = status != Gst.StateChangeReturn.FAILURE
        if not self.opened:
            self.pipeline.set_state(Gst.State.NULL)
            raise RuntimeError(f"failed to start H265 Y-plane pipeline: {pipeline_text}")
        self.source = pipeline_text
    ...
    def read(self):
        if not self.opened or self.sink is None:
            return False, None
        ...
        if not ok:
            return False, None
        try:
            raw = np.frombuffer(memoryview(info.data), dtype=np.uint8)
...
class H265GrayMp4Recorder:
    def __init__(self, path, width, height, fps, bps):
        self.path = str(path)
        self.width = int(width)
        self.height = int(height)
        self.fps = float(fps)
        ...
        try:
            ...
            raise RuntimeError(f"Python GStreamer unavailable for H265 recorder: {exc}") from exc
        Gst.init(None)
        self.Gst = Gst
        pipeline_text = (
            ...
            f"framerate={int(round(self.fps))}/1 ! "
            f"mpph265enc bps={self.bps} gop={int(round(self.fps))} ! "
            "h265parse ! mp4mux faststart=true ! "
            f"filesink location={self.path}"
        )
        self.pipeline = Gst.parse_launch(pipeline_text)
        self.appsrc = self.pipeline.get_by_name("src")
        if self.appsrc is None:
            raise RuntimeError("H265 recorder appsrc not found")
        self.duration_ns = int(round(1_000_000_000.0 / max(1.0, self.fps)))
        status = self.pipeline.set_state(Gst.State.PLAYING)
        if status == Gst.StateChangeReturn.FAILURE:
            self.pipeline.set_state(Gst.State.NULL)
            raise RuntimeError(f"failed to start H265 recorder pipeline: {pipeline_text}")
    ...
    def write(self, gray):
        ...
        uv_size = y_size // 2
        nv12 = np.empty(y_size + uv_size, dtype=np.uint8)
        nv12[:y_size] = gray.reshape(-1)
        nv12[y_size:] = 128
        data = nv12.tobytes()
        buf = self.Gst.Buffer.new_wrapped(data)
        pts = self.frame_count * self.duration_ns
        buf.pts = pts
        buf.dts = pts
        buf.duration = self.duration_ns
        ret = self.appsrc.emit("push-buffer", buf)
        if ret != self.Gst.FlowReturn.OK:
            raise RuntimeError(f"H265 recorder push-buffer failed: {ret}")
        self.frame_count += 1
...
def open_camera_capture(cam_idx):
    h265_source = get_h265_device_for_cam(int(cam_idx))
    if h265_source:
        source = h265_source
        cap = H265YPlaneCapture(source, CAPTURE_W, CAPTURE_H, H265_YPLANE_FPS)
        return cap, getattr(cap, "source", source)
    ...
    while not stop_event.is_set():
        ...
        for offset in range(N_CAM):
            ...
            try:
                ...
                if len(item) >= 8:
                    roi, x, y, src_frame_idx, enqueue_ts, seed_cx, seed_cy, roi_layer = item[:8]
                elif len(item) >= 7:
                    roi, x, y, src_frame_idx, enqueue_ts, seed_cx, seed_cy = item[:7]
                    roi_layer = "high" if HIGH_LAYER_MODE else "low"
                else:
...
def capture_job(cam_idx, initial_bg_gray=None, initial_bg_tol=None, initial_bg_qrange=None, layer_profile=None):
    ...
    if CAMERA_WARMUP_FRAMES > 0 and not SIMULATE_BY_VIDEOS:
        warmup_ok = 0
        for _ in range(CAMERA_WARMUP_FRAMES):
            ...
            if ret:
                ...
                time.sleep(0.02)
        print(
            f"--> Cam {cam_idx} camera warmup discarded frames={warmup_ok}/{CAMERA_WARMUP_FRAMES}",
            flush=True,
        )
    ...
    frame_t1 = None
    last_yolo_global_scan_frame = -YOLO_GLOBAL_SCAN_INTERVAL_FRAMES
    vibe_tile_detector = VibeTileDetector(DIFF_W, DIFF_H) if VIBE_TILE_MODE else None
    ...
    print(
        ...
        f"vibe_tile={'yes' if VIBE_TILE_MODE else 'no'} "
        f"vibe_tile_size={VIBE_TILE_W}x{VIBE_TILE_H}x{VIBE_TILE_MAX_ROIS} "
        f"global_local={'yes' if YOLO_GLOBAL_LOCAL_MODE else 'no'} "
        f"global_scan_interval={YOLO_GLOBAL_SCAN_INTERVAL_FRAMES} "
        f"local_roi={YOLO_LOCAL_ROI_CROP_SIZE}x{YOLO_LOCAL_ROI_MAX} "
    ...
    bg_static_point_mask = build_night_static_point_mask(bg_gray, bg_qrange, cam_profile)
    startup_star_learning_start_ts = None
    startup_star_mask = None
    startup_star_box_count = 0
    ...
    gray_snapshot_last_ts = 0.0
    gray_snapshot_count = 0
    if GRAY_SNAPSHOT_DIR:
        os.makedirs(GRAY_SNAPSHOT_DIR, exist_ok=True)
    gray_record_writer = None
    gray_record_start_ts = None
    gray_record_frame_count = 0
    gray_record_path = ""
    if GRAY_RECORD_DIR and GRAY_RECORD_SECONDS > 0.0:
        os.makedirs(GRAY_RECORD_DIR, exist_ok=True)
        stamp = time.strftime("%Y%m%d_%H%M%S")
        if GRAY_RECORD_MODE in ("h265", "h265_gray", "mp4"):
            gray_record_path = os.path.join(GRAY_RECORD_DIR, f"cam{cam_idx}_gray_y_to_h265_2k_{stamp}.mp4")
            gray_record_writer = H265GrayMp4Recorder(
                gray_record_path,
                CAPTURE_W,
                CAPTURE_H,
                GRAY_RECORD_FPS,
                GRAY_RECORD_H265_BPS,
            )
        elif GRAY_RECORD_MODE in ("y8", "raw", "single"):
            gray_record_path = os.path.join(GRAY_RECORD_DIR, f"cam{cam_idx}_single_y_2k_{stamp}.y8")
            gray_record_writer = Y8RawRecorder(gray_record_path, CAPTURE_W, CAPTURE_H)
        else:
            raise ValueError("UAV_GRAY_RECORD_MODE must be h265_gray or y8")
        if gray_record_writer is not None:
            print(
                f"--> Cam {cam_idx} gray record: mode={GRAY_RECORD_MODE} path={gray_record_path} "
                f"fps={GRAY_RECORD_FPS:.1f} seconds={GRAY_RECORD_SECONDS:.1f}",
                flush=True,
            )
    pixel_map_record_writer = None
    pixel_map_record_start_ts = None
    pixel_map_record_frame_count = 0
    pixel_map_record_path = ""
    if PIXEL_MAP_RECORD_DIR and PIXEL_MAP_RECORD_SECONDS > 0.0:
        os.makedirs(PIXEL_MAP_RECORD_DIR, exist_ok=True)
        stamp = time.strftime("%Y%m%d_%H%M%S")
        if PIXEL_MAP_RECORD_MODE in ("h265", "mp4"):
            pixel_map_record_path = os.path.join(PIXEL_MAP_RECORD_DIR, f"cam{cam_idx}_vibe_pixel_map_2k_{stamp}.mp4")
            pixel_map_record_writer = H265GrayMp4Recorder(
                pixel_map_record_path,
                DIFF_W,
                DIFF_H,
                PIXEL_MAP_RECORD_FPS,
                PIXEL_MAP_RECORD_BPS,
            )
        elif PIXEL_MAP_RECORD_MODE in ("y8", "raw", "single"):
            pixel_map_record_path = os.path.join(PIXEL_MAP_RECORD_DIR, f"cam{cam_idx}_vibe_pixel_map_2k_{stamp}.y8")
            pixel_map_record_writer = Y8RawRecorder(pixel_map_record_path, DIFF_W, DIFF_H)
        else:
            raise ValueError("UAV_PIXEL_MAP_RECORD_MODE must be h265 or y8")
        print(
            f"--> Cam {cam_idx} pixel-map record: mode={PIXEL_MAP_RECORD_MODE} "
            f"path={pixel_map_record_path} size={DIFF_W}x{DIFF_H} "
            f"fps={PIXEL_MAP_RECORD_FPS:.1f} seconds={PIXEL_MAP_RECORD_SECONDS:.1f}",
            flush=True,
        )
    ...
    def drain_inference_results(current_frame, current_frame_idx, full_w, full_h):
        ...
        nonlocal startup_star_mask, startup_star_box_count
        ...
        while not stop_event.is_set():
            ...
            det_profile = (
                ...
                else edge_high_profile
                if ENABLE_EDGE_REGION_LAYER and roi_layer == "high"
                else cam_profile
            )
            ...
            for det in dets:
                score = float(det[4]) if len(det) > 4 else float(det_profile["conf_thresh"])
                if score < float(det_profile["conf_thresh"]):
                    continue
                mapped_box = [
                ...
                if (
                    startup_star_suppress_enabled(det_profile)
                    and startup_star_learning_start_ts is not None
                    and startup_star_learning_active(startup_star_learning_start_ts)
                    and startup_star_mask is not None
                    and startup_star_candidate_box(mapped_box)
                ):
                    if add_startup_star_box(startup_star_mask, mapped_box, full_w, full_h):
                        startup_star_box_count += 1
                    continue
                if startup_star_box_hit(startup_star_mask, mapped_box, full_w, full_h):
                    continue
                if is_night_static_point_box(mapped_box, bg_static_point_mask, full_w, full_h, det_profile):
                    continue
                ...
                if (
                    ...
                    and is_valid_detection_box_size(mapped_box, det_profile)
    ...
    while not stop_event.is_set():
        ...
        if startup_star_suppress_enabled(cam_profile) and startup_star_mask is None:
            startup_star_mask = np.zeros((H, W), dtype=np.uint8)
            print(
                f"--> Cam {cam_idx} startup star suppress: learning "
                f"{NIGHT_STARTUP_STAR_SECONDS:.1f}s box<=({NIGHT_STARTUP_STAR_MAX_BOX_W}x"
                f"{NIGHT_STARTUP_STAR_MAX_BOX_H}, area={NIGHT_STARTUP_STAR_MAX_BOX_AREA})",
                flush=True,
            )
        if (
            startup_star_suppress_enabled(cam_profile)
            and startup_star_mask is not None
            and startup_star_learning_start_ts is None
            and bg_gray is not None
        ):
            startup_star_learning_start_ts = time.monotonic()
            print(f"--> Cam {cam_idx} startup star suppress learning window started.", flush=True)
        if (
            startup_star_suppress_enabled(cam_profile)
            and startup_star_mask is not None
            and startup_star_learning_start_ts is not None
            and not startup_star_reported_ready
            and not startup_star_learning_active(startup_star_learning_start_ts)
        ):
            startup_star_reported_ready = True
            star_pixels = cv2.countNonZero(startup_star_mask)
            print(
                f"--> Cam {cam_idx} startup star suppress ready: "
                f"boxes={startup_star_box_count} mask_pixels={star_pixels}",
                flush=True,
            )
        ...
        if GRAY_SNAPSHOT_DIR and GRAY_SNAPSHOT_EVERY_SEC > 0.0:
            snap_now = time.monotonic()
            if snap_now - gray_snapshot_last_ts >= GRAY_SNAPSHOT_EVERY_SEC:
                gray_snapshot_last_ts = snap_now
                gray_snapshot_count += 1
                snap_gray = gray_frame if gray_frame is not None else frame_to_gray(frame)
                latest_path = os.path.join(GRAY_SNAPSHOT_DIR, f"latest_cam{cam_idx}_gray.jpg")
                snap_path = os.path.join(
                    GRAY_SNAPSHOT_DIR,
                    f"cam{cam_idx}_gray_f{f_idx:06d}_{gray_snapshot_count:04d}.jpg",
                )
                try:
                    cv2.imwrite(latest_path, snap_gray)
                    cv2.imwrite(snap_path, snap_gray)
                except Exception as exc:
                    if gray_snapshot_count <= 3:
                        print(f"--> Cam {cam_idx} gray snapshot failed: {exc}", flush=True)
        if gray_record_writer is not None:
            record_now = time.monotonic()
            if gray_record_start_ts is None:
                gray_record_start_ts = record_now
            if record_now - gray_record_start_ts <= GRAY_RECORD_SECONDS:
                record_gray = gray_frame if gray_frame is not None else frame_to_gray(frame)
                if record_gray.shape[1] != CAPTURE_W or record_gray.shape[0] != CAPTURE_H:
                    record_gray = cv2.resize(record_gray, (CAPTURE_W, CAPTURE_H), interpolation=cv2.INTER_AREA)
                try:
                    gray_record_writer.write(record_gray)
                    gray_record_frame_count += 1
                except Exception as exc:
                    print(f"--> Cam {cam_idx} gray record failed: {exc}", flush=True)
                    gray_record_writer.release()
                    gray_record_writer = None
            else:
                gray_record_writer.release()
                gray_record_writer = None
                print(
                    f"--> Cam {cam_idx} gray record done: frames={gray_record_frame_count} "
                    f"path={gray_record_path}",
                    flush=True,
                )
        ...
        if (f_idx + cam_idx) % PROCESS_EVERY_N_FRAMES == 0:
            ...
            if ENABLE_LOW_LAYER_STATIC_BG_MASK and not VIBE_TILE_MODE and not YOLO_GLOBAL_LOCAL_MODE and bg_gray is None:
                now_ts = time.monotonic()
                ...
                bg_learn_seconds = online_static_bg_seconds(cam_profile)
                sample_interval = bg_learn_seconds / float(max(1, LOW_BG_SAMPLE_FRAMES))
                ...
                fallback_samples = learning_elapsed >= bg_learn_seconds + 2.0 and len(bg_samples) >= 20
                if learning_elapsed >= bg_learn_seconds and (enough_samples or fallback_samples):
                    bg_sample_count = len(bg_samples)
                    bg_gray, bg_tol, bg_qrange = build_static_bg_model(bg_samples)
                    bg_risk_model = build_background_risk_model(bg_qrange)
                    cam_profile = make_cam_profile_from_background(cam_idx, bg_gray, bg_qrange)
                    cam_profile["reason"] = f"online:{cam_profile.get('reason', '')}"
                    CAM_LAYER_PROFILES[cam_idx] = cam_profile
                    set_current_layer_profile(cam_profile)
                    tracker.profile = cam_profile
                    bg_static_point_mask = build_night_static_point_mask(bg_gray, bg_qrange, cam_profile)
                    bg_samples = []
                    static_point_count = cv2.countNonZero(bg_static_point_mask) if bg_static_point_mask is not None else 0
                    print(
                        f"--> Cam {cam_idx} own static background ready: "
                        f"seconds={learning_elapsed:.1f}/{bg_learn_seconds:.1f} samples={bg_sample_count} "
                        f"day={cam_profile.get('day_scene')} "
                        f"roi=native640+tight "
                        f"layer_conf={cam_profile.get('layer_confidence', 0.0):.2f} "
                        f"static_points={static_point_count}",
                        flush=True,
                    )
                frame_t1 = gray_small
                if not DIRECT_FULL_FRAME_INFERENCE:
                    if draw_this_frame:
                        try:
                            ...
                            if send_video_this_frame:
                                v_sender.send(BOARD_ID, cam_idx, show_frame)
            ...
            if VIBE_TILE_MODE:
                if VIBE_TILE_KEEP_TRACK_ROIS and ENABLE_TRAJECTORY_TRACKING:
                    track_rois = tracker.get_yolo_global_local_rois(
                        ...
                        W,
                        H,
                        max_rois=max(1, min(YOLO_LOCAL_ROI_MAX, VIBE_TILE_MAX_ROIS)),
                    )
                else:
                    track_rois = []
                vibe_rois, motion_mask = (
                    vibe_tile_detector.update_and_get_rois(gray_small, W, H, scale_x, scale_y)
                    if vibe_tile_detector is not None
                    else ([], None)
                )
                if track_rois:
                    track_roi_keys = {tuple(int(v) for v in r[:4]) for r in track_rois}
                rois = merge_nearby_boxes(track_rois + vibe_rois, dist_thresh=160)
                rois = suppress_overlapping_rois(rois)
                rois.sort(key=lambda r: r[4] if len(r) > 4 else 0.0, reverse=True)
                rois = prioritize_diverse_rois(rois, W, H, max_rois=MAX_ENQUEUED_ROIS_PER_UPDATE)

            if YOLO_GLOBAL_LOCAL_MODE:
                track_rois = (
                    tracker.get_yolo_global_local_rois(f_idx, W, H, max_rois=YOLO_LOCAL_ROI_MAX)
                    if ENABLE_TRAJECTORY_TRACKING
                    else []
                )
                if track_rois:
                    track_roi_keys = {tuple(int(v) for v in r[:4]) for r in track_rois}
                    rois.extend(track_rois)
                if f_idx - last_yolo_global_scan_frame >= YOLO_GLOBAL_SCAN_INTERVAL_FRAMES:
                    rois.insert(0, (0, 0, W, H, YOLO_GLOBAL_SCAN_SCORE))
                    last_yolo_global_scan_frame = f_idx
                rois = rois[:MAX_ENQUEUED_ROIS_PER_UPDATE]

            if (not VIBE_TILE_MODE) and (not YOLO_GLOBAL_LOCAL_MODE) and ENABLE_FRAME_DIFF_ROIS and frame_t1 is not None:
                with FRAME_DIFF_LOCK:
                    diff_img = cv2.absdiff(frame_t1, gray_small)
                    diff_rois = []
                    if ENABLE_EDGE_REGION_LAYER:
                        if (
                            edge_low_region_mask is None
                            or f_idx - edge_region_mask_frame >= EDGE_REGION_UPDATE_FRAMES
                        ):
                        ...
                        mask = cleanup_motion_mask_with_iters(
                            mask,
                            cam_profile.get("motion_erode_iter", MOTION_ERODE_ITER),
                            cam_profile.get("motion_dilate_iter", MOTION_DILATE_ITER),
                            cam_profile.get("motion_close_iter", MOTION_CLOSE_ITER),
                        )
                        mask = apply_night_static_point_mask(mask, bg_static_point_mask, cam_profile)
                        mask = apply_startup_star_mask(mask, startup_star_mask)
            ...
            if (not VIBE_TILE_MODE) and (not YOLO_GLOBAL_LOCAL_MODE) and ENABLE_FRAME_DIFF_ROIS and require_track_motion and track_rois:
                track_rois = [r for r in track_rois if track_roi_has_motion(r, motion_mask, W, H)]
            if (not VIBE_TILE_MODE) and (not YOLO_GLOBAL_LOCAL_MODE) and ENABLE_FRAME_DIFF_ROIS and (track_rois or hover_rois):
            ...
            if (not VIBE_TILE_MODE) and not YOLO_GLOBAL_LOCAL_MODE:
                rois = filter_night_static_point_rois(rois, bg_static_point_mask, W, H, cam_profile)
                rois = filter_startup_star_rois(rois, startup_star_mask, W, H)
            ...
            if VIBE_TILE_MODE:
                pass
            elif YOLO_GLOBAL_LOCAL_MODE:
                pass
            elif MOTION_ZOOM_ONLY:
            ...
            if pixel_map_record_writer is not None and motion_mask is not None:
                record_now = time.monotonic()
                if pixel_map_record_start_ts is None:
                    pixel_map_record_start_ts = record_now
                if record_now - pixel_map_record_start_ts <= PIXEL_MAP_RECORD_SECONDS:
                    pixel_map = motion_mask
                    if pixel_map.shape[1] != DIFF_W or pixel_map.shape[0] != DIFF_H:
                        pixel_map = cv2.resize(pixel_map, (DIFF_W, DIFF_H), interpolation=cv2.INTER_NEAREST)
                    try:
                        pixel_map_record_writer.write(pixel_map)
                        pixel_map_record_frame_count += 1
                    except Exception as exc:
                        print(f"--> Cam {cam_idx} pixel-map record failed: {exc}", flush=True)
                        pixel_map_record_writer.release()
                        pixel_map_record_writer = None
                else:
                    pixel_map_record_writer.release()
                    pixel_map_record_writer = None
                    print(
                        f"--> Cam {cam_idx} pixel-map record done: frames={pixel_map_record_frame_count} "
                        f"path={pixel_map_record_path}",
                        flush=True,
                    )
            ...
            for roi in rois[:MAX_ENQUEUED_ROIS_PER_UPDATE]:
                ...
                roi_layer = roi_layer_name(roi, cam_profile.get("name", "high"))
                roi_profile = (
                    edge_low_profile
                    if ENABLE_EDGE_REGION_LAYER and roi_layer == "low"
                    else edge_high_profile
                ...
                if roi_is_zoom_crop(roi):
                    ...
                    origin_x = int(x1)
                    origin_y = int(y1)
                elif roi_profile.get("enable_tight_motion_roi", False) and roi_is_padded_canvas(roi):
                ...
                elif len(roi_img.shape) == 3 and roi_img.shape[2] == 1:
                    roi_img = cv2.cvtColor(roi_img[:, :, 0], cv2.COLOR_GRAY2BGR)
                if put_latest(
                    inf_queues[cam_idx],
                    (roi_img, origin_x, origin_y, f_idx, enqueue_ts, seed_cx, seed_cy, roi_layer),
                ):
                    ...
                    if is_track_roi:
                        ...
                        label = "ROI"
                        color = (255, 255, 255)
                    if draw_this_frame and DRAW_INTERMEDIATE_BOXES:
                        current_draw_boxes.append([x1, y1, x2, y2, color, label, 1])
        ...
        for rb in unique_raw_boxes:
            if draw_this_frame and DRAW_INTERMEDIATE_BOXES:
                current_draw_boxes.append([rb[0], rb[1], rb[2], rb[3], (0, 0, 255), "Raw", 1])
        ...
        if draw_this_frame and len(current_draw_boxes) > MAX_DRAW_BOXES:
            current_draw_boxes = current_draw_boxes[-MAX_DRAW_BOXES:]

        if draw_this_frame:
            try:
                show_frame = cv2.resize(frame, (VIDEO_STREAM_W, VIDEO_STREAM_H), interpolation=cv2.INTER_AREA)
                if (not H265_YPLANE_KEEP_GRAY_PREVIEW) and len(show_frame.shape) == 2:
                    show_frame = cv2.cvtColor(show_frame, cv2.COLOR_GRAY2BGR)
                dsx, dsy = float(VIDEO_STREAM_W) / W, float(VIDEO_STREAM_H) / H
                if len(show_frame.shape) == 2 and current_draw_boxes and not H265_YPLANE_KEEP_GRAY_PREVIEW:
                    show_frame = cv2.cvtColor(show_frame, cv2.COLOR_GRAY2BGR)
                if len(show_frame.shape) == 2:
                    for x1, y1, x2, y2, _color, text, _life in current_draw_boxes:
                        cv2.rectangle(
                            show_frame,
                            (int(x1 * dsx), int(y1 * dsy)),
                            (int(x2 * dsx), int(y2 * dsy)),
                            255,
                            1 if text in ("ROI", "TRKROI") else 2,
                        )
                else:
                ...
                if SHOW_INDIVIDUAL_WINDOWS:
                    put_latest(display_queues[cam_idx], show_frame)
                current_draw_boxes.clear()
            except: pass
    cap.release()
    if gray_record_writer is not None:
        gray_record_writer.release()
    if pixel_map_record_writer is not None:
        pixel_map_record_writer.release()
    if gray_record_frame_count > 0:
        print(
            f"--> Cam {cam_idx} gray record summary: frames={gray_record_frame_count} "
            f"path={gray_record_path}",
            flush=True,
        )
    if pixel_map_record_frame_count > 0:
        print(
            f"--> Cam {cam_idx} pixel-map record summary: frames={pixel_map_record_frame_count} "
            f"path={pixel_map_record_path}",
            flush=True,
        )
    if output_writer is not None:
...
if __name__ == '__main__':
    ...
    if SIMULATE_BY_VIDEOS and ENABLE_LOW_LAYER_STATIC_BG_MASK and not VIBE_TILE_MODE and not YOLO_GLOBAL_LOCAL_MODE:
        if VIDEO_TEST_PATHS:
            built_backgrounds = []
            for cam_idx in range(N_CAM):
                source = get_video_test_source(cam_idx)
                ...
                try:
                    ...
                    if VIDEO_FORCE_LOW_BG_30 and needs_low_background and STATIC_BG_SECONDS < 30.0:
            ...
            try:
                ...
                if VIDEO_FORCE_LOW_BG_30 and not estimated_high and STATIC_BG_SECONDS < 30.0:
    ...
    elif (not SIMULATE_BY_VIDEOS) and ENABLE_LOW_LAYER_STATIC_BG_MASK and ENABLE_CAMERA_PREBUILD_BG and not VIBE_TILE_MODE and not YOLO_GLOBAL_LOCAL_MODE:
        built_backgrounds = []
        for cam_idx in range(N_CAM):
            try:
    ...
    if ENABLE_EDGE_REGION_LAYER:
        edge_high_profile, edge_low_profile = edge_region_profiles()
        profile_thresholds = [float(edge_high_profile["conf_thresh"]), float(edge_low_profile["conf_thresh"])]
        profile_thresholds.extend(float(p["conf_thresh"]) for p in CAM_LAYER_PROFILES)
        ACTIVE_YOLO_CONF_THRESH = min(profile_thresholds)
    else:
    ...
    print(
        ...
        f"vibe_tile={'yes' if VIBE_TILE_MODE else 'no'} "
        f"vibe_tile_size={VIBE_TILE_W}x{VIBE_TILE_H} "
        f"global_local={'yes' if YOLO_GLOBAL_LOCAL_MODE else 'no'} "
        f"global_interval={YOLO_GLOBAL_SCAN_INTERVAL_FRAMES} "
        f"local_roi={YOLO_LOCAL_ROI_CROP_SIZE} "
        f"direct_fullframe={'yes' if DIRECT_FULL_FRAME_INFERENCE else 'no'} "
    ...
    if ENABLE_NIGHT_STARTUP_STAR_SUPPRESS:
        print(
            f"--> Startup star suppression: seconds={NIGHT_STARTUP_STAR_SECONDS:.1f} "
            f"pad={NIGHT_STARTUP_STAR_PAD} max_box={NIGHT_STARTUP_STAR_MAX_BOX_W}x"
            f"{NIGHT_STARTUP_STAR_MAX_BOX_H} area={NIGHT_STARTUP_STAR_MAX_BOX_AREA}",
            flush=True,
        )
